...
### ❌ WRONG Examples (Will Cause Mojibake):
```python
plt.suptitle('経営サマリー')  # ❌ NO FONT!
ax.set_title('売上推移')  # ❌ NO FONT!
ax.text(0.5, 0.5, 'アラート')  # ❌ NO FONT!
```
...
ax.legend(['データ'], prop=JP_FP)  # ✅ Note: prop= not fontproperties=
```

### Automatic Validation

**Before ANY code generation with Japanese:**
1. Check if `JP_FP` is already defined in the file
2. If using matplotlib AND Japanese text → MUST add `fontproperties=JP_FP`
3. If legend → MUST use `prop=JP_FP`

### Common Mistakes to Avoid:
- ❌ Forgetting fontproperties in ax.text()
- ❌ Using fontproperties= in legend (should be prop=)
- ❌ Assuming font is inherited from plt.rcParams
- ❌ Only adding font to titles but not labels/annotations
