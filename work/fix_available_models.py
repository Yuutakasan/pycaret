...
try:
    ...
    for i, cell in enumerate(nb.cells):
        if cell.cell_type == 'code' and 'cuML GPU加速モデリング' in cell.source:
            cell.source = cuml_gpu_code
            print(f"✓ セル {i}: CatBoostを除外した版に更新しました")
            replaced = True
            break
    ...
    print("\n✅ 修正完了")
    print("\n使用するモデル:")
    print("  🚀 cuML GPU: Random Forest, Linear Regression, Ridge, Lasso, KNN, SVR")
    print("  💻 PyCaret: lr, ridge, lasso, rf, et, gbr, xgboost, lightgbm")
    print("  ❌ 除外: catboost (この環境では利用不可)")
